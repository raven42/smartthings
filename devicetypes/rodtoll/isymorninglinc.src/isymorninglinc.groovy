/**
 *  ISYMorningLinc - Handles a MorningLinc device connected to an ISY Hub. Uses only the main device which is on is lock. 
 *
...
metadata {
    definition (name: "ISYMorningLinc", namespace: "rodtoll", author: "Rod Toll") {
        capability "Lock"
        capability "Actuator"
...
def setISYState(address,state) {
	...
	log.debug 'ISYMORNINGLINC: Switching lock level to: '+state
...
// handle commands
def lock() {
    log.debug "ISYMORNINGLINC: Executing 'lock'"
    parent.on(this,device.deviceNetworkId)
}

def unlock() {
    log.debug "ISYMORNINGLINC: Executing 'unlock'"
